...
class Auth:
    ...
    async def combolist(self) -> list:
        b=[]
        for account in set(self.accounts):
            account = account.strip("\n").split(":")
            email = account[0]
            password = ":".join(account[1:])
            async with ClientSession() as sesh:
                async with sesh.get("https://login.live.com/oauth20_authorize.srf?client_id=000000004C12AE6F&redirect_uri=https://login.live.com/oauth20_desktop.srf&scope=service::user.auth.xboxlive.com::MBI_SSL&response_type=token") as r:
                    posturl = search("urlPost:'(.*?)'", (await r.text())).group(1)
    ...
    async def jwt(self) -> list:
        z = []
        for token in set(self.accounts):
            async with ClientSession() as sesh:
